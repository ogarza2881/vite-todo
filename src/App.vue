...
<ul class="space-y-4">
  ...
  class="bg-white rounded-xl text-3xl py-2 px-4 font-light text-green-800 text-center hover:text-white hover:bg-green-800 cursor-pointer transition-colors" @click="changeStatus(todo.id)">{{todo.text}}</li>
</ul>
  </section>
...
<ul class="space-y-4">
  <li 
  v-for = "todo in completedTodos" :key="todo.id"
  class="bg-white rounded-xl text-3xl py-2 px-4 font-light text-red-800 text-center hover:text-white hover:bg-red-800 cursor-pointer transition-colors" @click="changeStatus(todo.id)">{{todo.text}}</li>
  </ul>
  </section>
</main>
</div>
